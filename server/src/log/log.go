...
func New(output *os.File) *Logger {
	l := &Logger{flag: LstdFlags, level: Level_Debug, name: "", panicLevel: Level_Fatal, fileOutput: output}
...
func (self *Logger) Log(c Color, level Level, format string, v ...interface{}) {
	...
	prefix := fmt.Sprintf("[%d] %s", os.Getpid(), levelString[level])
...
func GetLogHaddle() *Logger {
	return &Tlog
}
func LogInit() int {
	if conf.LogPath == "" || conf.LogFileName == "" {
		fmt.Println("get LogPath error, the value is nil")
		os.Exit(1)
	}

	//init the log haddle
	fp, err := os.OpenFile(conf.LogFileName, os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
		fmt.Println("open logfilename error ", err)
		os.Exit(1)
	}
	New(fp)
	return 0
